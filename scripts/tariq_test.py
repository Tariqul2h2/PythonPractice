  ...
  "query": {
    "bool": {
      ...
      "filter": {
        "range": {
          "@timestamp": {
            "gte": "2021-10-28T23:00:00",
            "lte": "2021-10-29T03:59:59"
          }
        }
      }
    }
...
def to_csv(data, file_name):
    try:
        ...
        with open(file_name, 'w') as file:
            temp_data = [[d['_source'].get('@timestamp'), d['_source'].get('timestamp'), d['_source'].get('device_id')] for d in data]
            temp_data.insert(0, ['@timestamp', 'timestamp', 'device_id'])
