...
index_name = 'tmh_grid_prod_marketplace_exchange_from_lc_split_v*'
...
payload = {
    ...
    "query": {
      "bool": {
        "must": [
          {
            "match": {
              "client_id" : "ea7ea6bbb9f0b14c96f4244c093fae21.EVBB-V2G-B2-LogisticCenter"
            }
          }
        ],
        "must_not": {
          "range": {
            "measurands.meter_value_producing_kW": {
              "lte": 0
            }
          }
        },
    ...
    "_source":["@timestamp","client_id","measurands"]
}
scroll_payload = {
    "scroll": "1m",
    "scroll_id": None
...
def to_csv(data, file_name):
    try:
        ...
        with open(file_name, 'w') as file:
            temp_data = [[d['_source'].get('@timestamp'), d['_source'].get('client_id'), d['_source'].get('measurands')] for d in data]
            temp_data.insert(0, ['@timestamp', 'client_id', 'measurands'])
