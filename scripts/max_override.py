  ...
  "query": {
    "bool": {
      "must": [
        {
          "match": {
            "site_id": "b3956cae-3cef-42b9-9e08-c5bba5f555a4"
          }
        }
      ],
      "filter": {
...
def to_csv(data, file_name):
    try:
        ...
        with open(file_name, 'w') as file:
            temp_data = [[d['_source'].get('@timestamp'), d['_source'].get('evs'), d['_source'].get('override_max_limit')] for d in data]
            temp_data.insert(0, ['@timestamp', 'evs', "override_max_limit"])
