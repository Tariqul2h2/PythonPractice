...
def to_csv(data, file_name):
    try:
        ...
        with open(file_name, 'w') as file:
            temp_data = [[d['_source'].get('@timestamp'), d['_source'].get('charger_id'), d['_source'].get('action'), d['_source'].get('request')] for d in data]
            temp_data.insert(0, ['@timestamp', 'charger_id', 'action', 'request' ])
